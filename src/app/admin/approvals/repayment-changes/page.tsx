...
function TermsBlock({ label, terms, proposed = false }: { label: string; terms: Terms; proposed?: boolean }) {
  return <div className={`rounded-lg border p-3 ${proposed ? 'border-primary/30 bg-primary/5' : 'bg-muted/20'}`}><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{label}</div><div className="mt-2 font-medium">{terms.durationValue} {terms.durationUnit}</div><div className="text-sm text-muted-foreground">{terms.repaymentFrequency} repayments</div></div>;
}
...
export default function RepaymentChangesPage() {
  ...
  return <div><PageHeader title="Repayment Plan Changes" description="Review client requests to change future repayment frequency or remaining duration." icon={CalendarSync} />{loading ? <div className="grid gap-4 lg:grid-cols-2">{[1, 2].map((item) => <Skeleton key={item} className="h-72" />)}</div> : !requests?.length ? <Card><CardContent className="py-16 text-center text-muted-foreground"><Clock className="mx-auto mb-3 h-10 w-10" />No repayment-plan changes are awaiting approval.</CardContent></Card> : <div className="grid gap-4 lg:grid-cols-2">{requests.map((request) => <Card key={request.id}><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-lg">{request.dealName}</CardTitle><CardDescription>{request.clientName} · Requested {request.requestedAt?.toDate().toLocaleString('en-NG')}</CardDescription></div><Badge variant="outline">Pending</Badge></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><TermsBlock label="Current plan" terms={request.currentTerms} /><TermsBlock label="Proposed plan" terms={request.proposedTerms} proposed /></div><div className="rounded-lg border p-3 text-sm"><div className="font-medium">Client&apos;s reason</div><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{request.reason}</p></div><p className="text-xs text-muted-foreground">Approval preserves completed repayment records and redistributes only the unpaid contractual principal and profit over the new future schedule.</p><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => void process(request, 'Rejected')} disabled={Boolean(processingId)}><X className="mr-2 h-4 w-4" /> Reject</Button><Button onClick={() => void process(request, 'Approved')} disabled={Boolean(processingId)}>{processingId === request.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />} Approve Revised Plan</Button></div></CardContent></Card>)}</div>}</div>;
}
