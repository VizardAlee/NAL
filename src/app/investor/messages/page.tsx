...
function ConversationItem({ conversation, currentUserId }: { conversation: Conversation, currentUserId: string }) {
    ...
    return (
        <Card
            ...
            onClick={() => router.push(`/investor/messages/${conversation.id}`)}
        >
...
export default function InvestorMessagesPage() {
    ...
    return (
        <div>
            <PageHeader title="Messages" description="Your conversations with the admin." icon={MessageSquare} />
                    ...
                    <Card className="h-48 flex flex-col items-center justify-center border-dashed">
                        <p className="text-muted-foreground">You have no active conversations.</p>
                         <Button asChild variant="link">
                            <Link href="/investor/dashboard">Back to Dashboard</Link>
                        </Button>
                    </Card>
                )}
            </div>
