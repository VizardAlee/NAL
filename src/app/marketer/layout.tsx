...
export default function MarketerLayout({
  ...
  useEffect(() => {
    ...
    if (!loading && user) {
      ...
      setStoredActivePortal('marketer', user.uid);
    }
  }, [user, loading, router]);
  

  if (loading || !user || logoLoading) {
    return <MarketerSkeleton />;
  }
  ...
  return (
    <div className="app-shell flex w-full flex-col">
        <header className="app-topbar sticky top-0 z-10 flex h-16 items-center gap-2 px-3 lg:gap-4 lg:px-6">
            <Link href="/marketer/dashboard" className="flex min-w-0 items-center gap-2 font-bold font-headline text-primary">
