...
export default function ClientKafaalahBondPage() {
  ...
  const download = () => {
    ...
    startDownload(async () => {
      const bytes = await buildKafaalahBondPdf(bond);
  ...
  return (
    <div className="agreement-screen mx-auto max-w-5xl">
      <div className="mb-5 flex flex-col gap-3 rounded-xl border bg-background p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between print:hidden"><div><div className="flex items-center gap-2"><h1 className="font-semibold">Kafaalah Guarantee and Indemnity Bond</h1><Badge variant="outline">{bond.version}</Badge></div><p className="font-mono text-xs text-muted-foreground">{bond.bondId}</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => window.print()} disabled={!canExport}><Printer className="mr-2 h-4 w-4" /> Print</Button><Button onClick={download} disabled={!canExport || downloading}>{downloading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />} Download PDF</Button></div></div>
      ...
      <article id="printable-agreement" className="agreement-paper bg-white px-8 py-7 text-[13px] leading-[1.55] text-slate-950 shadow-xl sm:px-14 sm:py-10">
        <header className="mb-6 flex items-center gap-4 border-b-2 border-[#075a3c] pb-4"><img src="/NAL%20LOGO.jpg" alt="NAL logo" className="h-16 w-20 rounded object-cover" /><div><div className="font-serif text-lg font-bold text-[#075a3c]">{bond.company.name}</div><div className="max-w-xl text-[11px] text-slate-600">{bond.company.address}</div></div></header>
        ...
        <section className="agreement-clause"><h3>EXECUTION</h3><p>DATED this {formatAgreementDate(bond.bondDate)}.</p><p>IN WITNESS WHEREOF, the Guarantor has executed this Bond on the date stated above.</p><div className="mt-5 grid gap-8 sm:grid-cols-2"><div><strong>SIGNED BY THE GUARANTOR</strong><p className="mt-3">Name: {bond.guarantor.name.toUpperCase()}<br />Capacity: Guarantor / Kafeel<br />Phone: {bond.guarantor.phoneNumber}<br />Occupation: {bond.guarantor.occupation}<br /><br />Signature: ________________________<br />Date: ____________________________</p></div><div><strong>IN THE PRESENCE OF A WITNESS</strong><p className="mt-3">Name: ____________________________<br />Phone Number: _____________________<br />Address: __________________________<br />Occupation: _______________________<br />Signature: ________________________<br />Date: ____________________________</p></div><AgreementCompanyStamp /></div></section>
