  ...
  return (
    <div className="agreement-screen mx-auto max-w-5xl">
      ...
      <article id="printable-agreement" className="agreement-paper bg-white px-8 py-7 text-[13px] leading-[1.55] text-slate-950 shadow-xl sm:px-14 sm:py-10">
        ...
        <h3 className="agreement-heading">AND</h3><div className="mb-4 flex items-start gap-4"><p className="flex-1"><strong>{legalPartyIntroduction(agreement.client, 'Customer')}</strong>, also acting as the procurement Agent, which expression shall, where the context permits, include its lawful successors and permitted assigns.</p>{agreement.client.photoURL && <img src={agreement.client.photoURL} alt={legalPartySignerName(agreement.client)} className="h-24 w-20 rounded border object-cover" />}</div>
